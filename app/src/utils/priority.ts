...
export function calculatePriority(task: Task): PriorityScore {
  ...
  // Main task bonus (main tasks are more certain/important)
  ...
  // Quick win bonus (tasks < 30min get a small boost for quick clearance)
  if (task.estimatedMinutes <= 30 && task.priority !== 'P0') {
    score += 8
    reasons.push('快速任务')
  }

  // In-progress bonus (continue what you started)
