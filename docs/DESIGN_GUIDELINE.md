...
当操作系统启用深色外观时，应用通过 `prefers-color-scheme: dark` 自动切换；浅色模式沿用本节之前定义的配色。
...
| 提醒 / P2 | `#B79A6D` |
| 普通 / P3 | `#9B978F` |
| 主线看板 | `#302E2A` → `#34312D`，边框 `#655B4C` |
| 支线看板 | `#302E2A` → `#35312C`，边框 `#6A5B49` |

深色模式采用中性暖炭灰作为主要表面，避免页面整体偏蓝或偏绿。规划、日报、周报、月报等长文区域以中性表面承载内容；正文保持柔和的暖白色，Markdown 正文和列表字号为 15px、行高为 1.75。状态色仅用于文字、图标、徽标和窄色条，不大面积铺设彩色背景；状态同时使用文字或图标表达。
